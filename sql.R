...
countryName <<- dbGetQuery(con, "SELECT rtrim(name) as name FROM countries ORDER BY name")
...
changeCountryView <- function(country)
{
  con <- dbConnect(drv, user=' ', host='localhost', port='5432', dbname='final_project', password=' ')
  #con <- dbConnect(drv, dbname="final_project")
  newView <- dbGetQuery(con, paste("SELECT lat, lon FROM country_centroid 
                                   WHERE name = '", country, "'", sep = ""))
  dbDisconnect(con)
  return(newView)
}
